import {getRPC as $cStG9$getRPC, methods as $cStG9$methods} from "@metavatem/aidpcoin-rpc";
import $cStG9$metavatemaidpcoinkey from "@metavatem/aidpcoin-key";
import $cStG9$metavatemaidpcoinsigntransaction from "@metavatem/aidpcoin-sign-transaction";
...
(0, $cStG9$metavatemaidpcoinsigntransaction).sign; //"Idiocracy" but prevents bundle tools such as PARCEL to strip this dependency out on build.
...
async function $67c46d86d9d50c48$export$322a62cff28f560a(WIF, wallet, onlineMode) {
    const privateKey = (0, $cStG9$metavatemaidpcoinkey).getAddressByWIF(wallet.network, WIF);
    const result = {};
    ...
    const signedHex = (0, $cStG9$metavatemaidpcoinsigntransaction).sign(wallet.network, rawHex, UTXOs, privateKeys);
...
async function $2767f256fef0b24e$export$df96cd8d56be0ab1(wallet, addresses) {
    ...
    const balance = await wallet.rpc((0, $cStG9$methods).getaddressbalance, params);
...
const $d7a766f9899f0fc1$var$URL_AIDPCOIN_MAINNET = "https://aidp-rpc-mainnet.ting.finance/rpc";
const $d7a766f9899f0fc1$var$URL_AIDPCOIN_TESTNET = "https://aidp-rpc-testnet.ting.finance/rpc";
const $d7a766f9899f0fc1$var$URL_EVRMORE_MAINNET = "https://evr-rpc-mainnet.ting.finance/rpc";
class $d7a766f9899f0fc1$export$bcca3ea514774656 {
   ...
   */ sweep(WIF, onlineMode) {
    ...
    async init(options) {
        ...
        let url = $d7a766f9899f0fc1$var$URL_AIDPCOIN_MAINNET;
        //VALIDATION
        ...
        if (options.network === "aidp-test") url = $d7a766f9899f0fc1$var$URL_AIDPCOIN_TESTNET;
        if (options.network === "evr") url = $d7a766f9899f0fc1$var$URL_EVRMORE_MAINNET;
        ...
        this.rpc = (0, $cStG9$getRPC)(username, password, url);
        this._mnemonic = options.mnemonic;
        //Generating the hd key is slow, so we re-use the object
        const hdKey = (0, $cStG9$metavatemaidpcoinkey).getHDKey(this.network, this._mnemonic);
        const coinType = (0, $cStG9$metavatemaidpcoinkey).getCoinType(this.network);
        const ACCOUNT = 0;
        ...
        while(doneDerivingAddresses === false){
            ...
            const tempAddresses = [];
            for(let i = 0; i < 20; i++){
                const external = (0, $cStG9$metavatemaidpcoinkey).getAddressByPath(this.network, hdKey, `m/44'/${coinType}'/${ACCOUNT}'/0/${this.addressPosition}`);
                const internal = (0, $cStG9$metavatemaidpcoinkey).getAddressByPath(this.network, hdKey, `m/44'/${coinType}'/${ACCOUNT}'/1/${this.addressPosition}`);
    ...
    async hasHistory(addresses) {
        ...
        const asdf = await this.rpc((0, $cStG9$methods).getaddressbalance, [
            obj,
            includeAssets
        ]);
        //@ts-ignore
    ...
    async getHistory() {
        ...
        const deltas = this.rpc((0, $cStG9$methods).getaddressdeltas, [
            {
                addresses: addresses,
                assetName: assetName
            }
    ...
    async getMempool() {
        const method = (0, $cStG9$methods).getaddressmempool;
        const includeAssets = true;
        const params = [
            {
   ...
   */ async getAssetUTXOs(assetName) {
        ...
        return this.rpc((0, $cStG9$methods).getaddressutxos, params);
    }
    async getUTXOs() {
        return this.rpc((0, $cStG9$methods).getaddressutxos, [
            {
                addresses: this.getAddresses()
            }
        ]);
   ...
   */ async createTransaction(options) {
        ...
        const signed = (0, $cStG9$metavatemaidpcoinsigntransaction).sign(this.network, raw, transaction.getUTXOs(), privateKeys);
        try {
   ...
   */ async getUTXOsInMempool(mempool) {
    ...
    constructor(){
        this.rpc = (0, $cStG9$getRPC)("anonymous", "anonymous", $d7a766f9899f0fc1$var$URL_AIDPCOIN_MAINNET);
        this._mnemonic = "";
        this.network = "aidp";
        this.addressObjects = [];
        this.receiveAddress = "";
...
var $d7a766f9899f0fc1$export$2e2bcd8739ae039 = {
    createInstance: $d7a766f9899f0fc1$export$99152e8d49ca4e7d,
    getBaseCurrencyByNetwork: $b986b05eb12d057a$export$af0c167f1aa2328f
};
async function $d7a766f9899f0fc1$export$99152e8d49ca4e7d(options) {
    const wallet = new $d7a766f9899f0fc1$export$bcca3ea514774656();
    await wallet.init(options);
    return wallet;
}


export {$d7a766f9899f0fc1$export$bcca3ea514774656 as Wallet, $d7a766f9899f0fc1$export$2e2bcd8739ae039 as default, $d7a766f9899f0fc1$export$99152e8d49ca4e7d as createInstance, $c3dba3dbad356cd6$export$febc5573c75cefb0 as Transaction, $c7db79d953d79f02$export$a0aa368c31ae6e6c as SendManyTransaction};
